...
def load(dll, check=check_non_zero):
    def deco(f):
        ...
        if not (
            ...
            raise TypeError("return_annotation must be a ctypes datatype or None for void", sig.return_annotation)
        ...
        for p in sig.parameters.values():
            ...
            if not isinstance(p.annotation, c_int.__mro__):
                raise TypeError("annotation must be a ctypes datatype or None for void", p)
...
def MessageBoxW(hWnd: HWND, lpText:LPCWSTR, lpCaption:LPCWSTR, uType:UINT) -> c_int: pass
